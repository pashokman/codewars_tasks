...
Each book has a code c of 3, 4, 5 or more characters. The 1st character of a code is a capital 
...
def stock_list(list_of_art, list_of_cat):
    category_counts = {cat: 0 for cat in list_of_cat}
    ...
    for item in list_of_art:
        ...
        if category[0] in category_counts:
            category_counts[category[0]] += int(count)
    
    return " - ".join([f"({cat} : {category_counts[cat]})" for cat in list_of_cat])
